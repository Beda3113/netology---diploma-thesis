...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            required=True,
            help='ID пользователя-поставщика'
        )
        parser.add_argument(
            '--file',
            type=str,
            help='Путь к локальному YAML файлу'
        )
        parser.add_argument(
            '--url',
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stderr.write(self.style.ERROR(f'Пользователь с ID {supplier_id} не найден или не является поставщиком'))
            return
        ...
        if file_path and yaml_url:
            self.stderr.write(self.style.ERROR('Укажите только один источник данных (--file ИЛИ --url)'))
            return
        ...
        try:
            if file_path:
                ...
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    self.stdout.write(self.style.SUCCESS(f'Успешно импортировано из файла {file_path}'))
            else:
                # Обработка URL
